...
def extract_document(
    ...
    with pdfplumber.open(pdf_path) as plumber_doc:
        for index in range(fitz_doc.page_count):
            ...
            # **خلل حقيقي مُصحَّح هنا (كان مقصوراً على `PageSource.SCANNED` فقط):**
            # صفحة رقمية بصور مُضمَّنة كبيرة (`_page_images`، خصوصاً بعد إضافة تركيب
            # RGBA/SMask) تخصّص كائنات Pillow/numpy ثقيلة تماماً كالصفحة الممسوحة —
            # ملف ~17MB حقيقي استغرق ~35 ثانية فقط قبل الانهيار (سريع جداً ليكون OCR
            # عبر Vision، يعني الأرجح صفحات رقمية بصور كبيرة مُضمَّنة) واستمر بالانهيار
            # رغم هذا التنظيف لأنه لم يكن يُشغَّل إطلاقاً لصفحاته. الآن يعمل على **كل**
            # صفحة بصرف النظر عن مصدرها — لتفادي بقاء ذروة الذاكرة مرتفعة طوال بقية
            # الاستخراج، وإعادة الذاكرة المُحرَّرة فعلياً لنظام التشغيل (انظر توثيق
            # `_release_freed_memory_to_os` لسبب الحاجة لهذه الخطوة تحديداً بعد
