...
class ImageAsset(BaseModel):
    ...
    مكتبة الوسائط الجانبية بالواجهة، ولإدراج Placeholder نصي تلقائي في مكانه الأصلي ضمن
    تدفّق المستند (`_insert_image_placeholders` في `ingest.py`) — الاستخراج مقصور على
    الصفحات الرقمية لأن الصفحة الممسوحة بالكامل هي نفسها صورة واحدة كبيرة (تُستخدم أصلاً
    كمدخل OCR)، وليست "شكلاً" مضمَّناً بالمعنى المقصود هنا.

    `image_id` (مثال "Image_01") تسلسلي عبر المستند كله بترتيب الاكتشاف، يُستخدَم حرفياً
    في نص الـplaceholder وفي اسم ملف PNG عند التصدير كي يجد المترجم/فريق DTP الصورة
    بسهولة. `bbox` يحدّد موضعها الأصلي في الصفحة لترتيب الـplaceholder بين الفقرات
    والجداول بحسب الموضع الرأسي الحقيقي، وليس بإلحاقه في نهاية الصفحة فقط."""
