...
class ImageAsset(BaseModel):
    """صورة/رسم بياني مُستخرَج من صفحة رقمية (get_images في PyMuPDF) أو من صفحة ممسوحة
    (اكتُشف هندسياً داخل بيتماب الصفحة عبر `_detect_scanned_photo_regions` في
    `ingest.py`)، لعرضه في مكتبة الوسائط الجانبية بالواجهة، ولإدراج Placeholder نصي
    تلقائي في مكانه الأصلي ضمن تدفّق المستند (`_insert_image_placeholders`).
