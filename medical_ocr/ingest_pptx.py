...
from .ingest import _downscale_if_huge, _flatten_to_white_rgb, _sort_blocks_by_position
...
def _slide_blocks_and_images(slide, slide_number: int, images: List[ImageAsset]) -> List[Block]:
    ...
    for shape in slide.shapes:
        ...
        if shape.shape_type == MSO_SHAPE_TYPE.PICTURE:
            try:
                png_buffer = BytesIO()
                image = _flatten_to_white_rgb(_downscale_if_huge(Image.open(BytesIO(shape.image.blob))))
