...
- بوابة إضافية مخصَّصة للرموز الطبية (اليوم السادس): `numeric_tokens_preserved`
  تمنع تحديداً تغيير رقم واضح (جرعة/قيمة) صراحةً، وهو خطر هلوسة لا يلتقطه
  التشابه النصي العام في `is_correction_grounded` (رقم مكتوب بوضوح تام قد
  يتغيّر بحرف واحد مع بقاء نسبة تشابه النص الكلي أعلى من الحد الأدنى).
"""
...
def numeric_tokens_preserved(raw_text: str, corrected_text: str) -> bool:
    """بوابة حماية الرموز الطبية (اليوم السادس): كل رقم صريح صرف في raw_text
    (جرعة، قيمة مخبرية، ...) يجب أن يظهر بنفس القيمة في corrected_text.

    لا تُقيَّد أرقام OCR الملتبسة بحروف (مثل "2O"، "5oo") لأنها ليست أرقاماً
    صرفة في raw_text أصلاً — تصحيحها إلى الرقم المقروء هو الغرض المقصود.
    """
    raw_numbers = Counter(extract_pure_numbers(raw_text))
    corrected_numbers = Counter(extract_pure_numbers(corrected_text))
    return all(corrected_numbers[number] >= count for number, count in raw_numbers.items())
...
def is_correction_grounded(
    ...
    if not (char_similarity >= min_char_similarity and word_count_delta <= max_extra_words):
        return False
    return numeric_tokens_preserved(raw_text, corrected_text)
