...
## 15. طلب مباشر بعد اليوم السابع — أول خط استخراج PDF كامل + واجهة Streamlit

- **السبب:** طلب مباشر من المستخدم لواجهة تفاعلية بسيطة "ارفع PDF وشاهد النتيجة الآن" —
  خارج تسلسل "اليوم N" المعتاد، ولم يكن أي مسار "من ملف PDF إلى Document" مبنياً بعد رغم
  وجود المحوّلات الجزئية في `medical_ocr/schema.py` منذ اليوم الثاني/الثالث.
- **`medical_ocr/ingest.py` (جديد، مكتمل ✅):** `extract_document(pdf_path, file_name=None) -> Document`.
  لكل صفحة: إن استخرج PyMuPDF نصاً ≥ 20 حرفاً تُعامَل كصفحة رقمية (فقرات عبر
  `page.get_text("blocks")` + جداول عبر `pdfplumber.find_tables()`)، وإلا تُعامَل كممسوحة
  (raster لكامل الصفحة عبر `page.get_pixmap()` ثم `easyocr.Reader(["ar","en"]).readtext(...)`).
  **تبسيط متعمد** عن تصميم القسم 3-ب الأصلي (تحقق مزدوج PaddleOCR+easyocr على الصفحات
  الممسوحة) — هنا محرك واحد فقط (easyocr)، لأن الهدف كان عرضاً تفاعلياً فعلياً بسرعة، وليس
  خط الإنتاج الكامل. **إن احتيج للتحقق المزدوج لاحقاً فهو لم يُبنَ بعد.**
- **`streamlit_app.py` (جديد، جذر المشروع، مكتمل ✅):** رفع PDF → استخراج فوري (مجاني تماماً،
  بلا LM) لكل الصفحات/الفقرات/الجداول تلقائياً → **زر منفصل لكل فقرة/جدول** لتفعيل التصحيح
  الإملائي أو هيكلة الجدول بالذكاء الاصطناعي، وليس تلقائياً للملف كاملاً. **قرار مقصود** بسبب
  محدودية حصة Gemini المجانية اليومية الموثّقة في القسم 14 — الهدف ألا يستهلك رفع ملف واحد
  الحصة كاملة دون قصد المستخدم. الشريط الجانبي يعرض حالة اتصال LM (متصل/غير متصل) بوضوح.
  يعمل الاستخراج والعرض حتى بلا `GEMINI_API_KEY` (تُعطَّل أزرار الذكاء الاصطناعي فقط).
- **معالجة الأخطاء:** ملف PDF تالف أو غير صالح رغم امتداده يُلتقَط برسالة عربية واضحة
  (`st.error`) بدل traceback خام يوقف الواجهة.
- **اختبار فعلي حقيقي (وليس بنيوياً فقط):**
  - مسار رقمي: PDF حقيقي بنص → استخراج ناجح بلا OCR.
  - مسار ممسوح: PDF بصفحة صورة بلا طبقة نص → تحميل أوزان `easyocr` فعلياً وتشغيله بنجاح
    (جودة التعرّف نفسها ضعيفة على نص تجريبي اصطناعي بخط PIL افتراضي — قيد بيانات الاختبار
    لا عطل في الكود).
  - مسار الجداول: PDF بجدول حقيقي بخطوط شبكة (`page.draw_line`) → `pdfplumber.find_tables()`
    يستخرج الصفوف بدقة كاملة.
  - ملف تالف: نص عادي بامتداد `.pdf` → استثناء `fitz.FileDataError` مُلتقَط بنجاح عبر نفس
    مسار المعالجة في `streamlit_app.py`.
  - زر "صحّح بالذكاء الاصطناعي" (نفس الدالة التي يستدعيها فعلياً): استدعاء حي ضد Gemini
    نجح (صحّح "ميتفوبرولين 5O" إلى "ميتوبرولول 50" بنفس منطق `is_correction_grounded`).
  - تشغيل الخادم فعلياً عبر `streamlit run` مرتين (قبل وبعد إضافة معالجة الأخطاء)، تحقق عبر
    `curl` من استجابة `200` بلا أخطاء استيراد في السجل، ثم إيقاف الخادم.
- **`tests/test_ingest.py` (جديد، مكتمل ✅):** 3 اختبارات — نص رقمي، جدول حقيقي عبر
  pdfplumber، وتوجيه صفحة فارغة لمسار OCR الممسوح (بـ mock لتفادي تحميل نموذج easyocr
  الحقيقي داخل مجموعة الاختبارات الآلية). **المجموع الآن 71 اختباراً ناجحاً.**
- **README.md محدَّث:** أُزيلت فقرة "غير مبني بعد: رفع ملف PDF كامل" لأنها لم تعد صحيحة،
  أُضيف قسم "تشغيل واجهة Streamlit"، وحُدِّث عدد الاختبارات من 69 إلى 71.
- **لم يتغيّر:** مبدأ منع الهلوسة والبوابات البرمجية مستقلة تماماً عن إضافة `ingest.py` —
  التصحيح بالذكاء الاصطناعي في الواجهة يستدعي نفس `MedicalSpellingCorrector`/
  `MedicalTableStructurer` المُختبَرين مسبقاً، دون أي تعديل على منطقهما الداخلي.
- **التالي:** التحقق المزدوج الكامل (PaddleOCR + easyocr) على الصفحات الممسوحة إن احتيج
  لدقة أعلى، ومرحلة Triage تلقائية أذكى من عتبة طول النص الحالية لتمييز الصفحة الرقمية
  من الممسوحة.

### 15.1 تجهيز للنشر على Streamlit Community Cloud (طلب رابط عام للتجربة غداً)

- **السبب:** المستخدم طلب رابط ويب عام (وليس تشغيلاً محلياً فقط) لتجربة رفع PDF غداً.
  النشر الفعلي (ربط GitHub بحساب Streamlit Cloud) يتطلب إجراء حساب شخصي (OAuth) لا يمكن
  تنفيذه من هذه الجلسة — تم فقط تجهيز المستودع ليكون النشر خطوات قليلة غداً.
- **`requirements.txt` (مكتمل ✅):** أُزيلا `paddlepaddle`/`paddleocr` منه عمداً — غير
  مستوردَين فعلياً في أي كود حالياً (تحقّق عبر `grep`)، وثقيلان جداً (paddlepaddle خاصة)
  لبناء سريع على حصة موارد Streamlit Community Cloud المجانية المحدودة. يُعادان لاحقاً
  عند بناء التحقق المزدوج الفعلي (القسم 3-ب).
- **`streamlit_app.py` — جسر `st.secrets` → متغيرات البيئة (مكتمل ✅):** على Streamlit
  Cloud تُضبط المفاتيح عبر لوحة "Secrets" وليس ملف `.env` محلي. أُضيفت `_load_secrets_into_env()`
  التي تنسخ `GEMINI_API_KEY`/`GOOGLE_API_KEY`/`MEDICAL_OCR_LM_MODEL` من `st.secrets` إلى
  `os.environ` إن وُجدت هناك ولم تكن موجودة أصلاً في البيئة — بلا أي تعديل على `lm_config.py`
  نفسه (يبقى يقرأ `os.getenv` فقط). محاطة بـ `try/except` عام لأن `st.secrets` قد لا يوجد
  محلياً بلا `secrets.toml`. اختُبر محلياً بلا أي `secrets.toml` — الخادم يقلع بلا أخطاء
  (يعتمد على `.env` كالمعتاد محلياً).
- **إعادة تشغيل الخادم والاختبارات (مكتمل ✅):** `streamlit run` يقلع بلا أخطاء بعد
  التعديلين، و72 اختباراً ما زالت ناجحة (لا تغيير في عددها — التعديلات هنا بنية نشر فقط).
- **التالي الفعلي (يدوي، غداً):** المستخدم يسجّل دخول على share.streamlit.io عبر GitHub،
  يُخوّل الوصول للمستودع الخاص `rabueshy-blip/medical-ocr-saas`، ينشر `streamlit_app.py`،
  ويضيف `GEMINI_API_KEY` في إعدادات Secrets (صيغة TOML: `GEMINI_API_KEY = "..."`).

## 16. التبديل من easyocr إلى Google Vision API كمحرك OCR أساسي للصفحات الممسوحة

- **السبب:** طلب مباشر لجعل خط الاستخراج يعتمد على Google Vision API (بدل easyocr) كخادم
  استخراج خلفي، بحثاً عن دقة أعلى على المستندات الطبية، مع بقاء منطق التصحيح/الترسيخ
  (DSPy + بوابات منع الهلوسة) كما هو تماماً دون أي تعديل.
- **`medical_ocr/ingest.py` (مكتمل ✅):**
  - `_call_vision_api()`: استدعاء REST مباشر (`requests`، بلا حزمة `google-cloud-vision`
    الثقيلة ولا ملف اعتماد خدمة — مفتاح API فقط، نفس فلسفة `GEMINI_API_KEY`) لميزة
    `DOCUMENT_TEXT_DETECTION` تحديداً (وليس `TEXT_DETECTION`) — مناسبة لمستندات كثيفة
    وتعطي تجميعاً هرمياً page/block/paragraph/word/symbol.
  - إعادة محاولة (retry) بحد أقصى 3 محاولات وتأخير تصاعدي، لكن **فقط** على أخطاء اتصال/
    timeout أو HTTP 5xx — أخطاء 4xx (مفتاح غير صالح) تفشل من أول محاولة عمداً (إعادة
    محاولتها لن تُصلحها).
  - `_scanned_page_blocks_vision()` + `_blocks_from_vision_page()`: يحوّلان استجابة Vision
    إلى `Block` من نوع paragraph (لا يوجد اكتشاف بنية جدول من Vision API — قيد معروف،
    الجداول الممسوحة تبقى بند مفتوح كما في القسم 3-ب-4؛ الجداول الرقمية عبر pdfplumber
    غير متأثرة).
  - `extract_document()`: فشل صفحة ممسوحة واحدة (بعد استنفاد إعادة المحاولة) لا يوقف بقية
    المستند — تُسجَّل بـ Block واحد واضح الفشل (confidence=0.0) بدل استثناء غير مُلتقَط.
  - `_scanned_page_blocks_easyocr()` (سابقاً `_scanned_page_blocks`): محفوظة كمحرك ثانٍ
    محلي غير مستخدَم افتراضياً، لغرض التحقق المزدوج المخطَّط في القسم 3-ب.
- **الاعتماديات:** أُضيفت `requests` صراحة إلى `requirements.txt` (كانت اعتمادية ضمنية فقط).
- **بيئة/نشر:** `GOOGLE_VISION_API_KEY` جديد في `.env.example`، مع توضيح صريح أنه **يتطلب
  تفعيل Billing على مشروع GCP (بطاقة دفع) حتى ضمن الحصة المجانية** — بخلاف `GEMINI_API_KEY`
  الذي لا يحتاج بطاقة إطلاقاً. جُسِّر أيضاً إلى `streamlit_app.py::_load_secrets_into_env()`
  للعمل على Streamlit Community Cloud عبر Secrets.
- **اختبارات جديدة (مكتمل ✅):** توسعة `tests/test_ingest.py` — مفتاح مفقود، إعادة محاولة
  ناجحة بعد خطأ اتصال/خادم، عدم إعادة محاولة على 4xx، فشل بعد استنفاد المحاولات، خطأ ضمن
  استجابة 200، تحويل paragraphs إلى Blocks بمتوسط confidence صحيح، وعدم توقف
  `extract_document` عند فشل صفحة واحدة. كل الاستدعاءات مُموَّهة (mock) بلا أي شبكة حقيقية.
  المجموع الآن **82 اختباراً**، كلها ناجحة (كان 72).
- **لم يتغيّر:** بوابات منع الهلوسة (`is_correction_grounded`, `row_values_grounded`,
  `numeric_guard.py`) ومنطق DSPy بالكامل — هذا التعديل محصور بخط الاستخراج (Triage/OCR)
  فقط، قبل أي تصحيح لاحق.
- **التالي:** تشغيل فعلي حي ضد Google Vision API حقيقي (لم يحدث بعد في هذه البيئة — يتطلب
  مفتاح API فعلي ومشروع GCP مفعَّل الفوترة)، ثم مقارنة يدوية للدقة مقابل easyocr على نفس
  عيّنات الاختبار. اكتشاف بنية الجداول الممسوحة (Table Transformer/PP-Structure) يبقى بنداً
  مفتوحاً كما في القسم 3-ب-4.
